...
public class PlayerNetworkSetup : MonoBehaviourPunCallbacks
{
    ...
    void Start()
    {
        if (photonView.IsMine)
        {
            ...
            object Avatar_Selection_Number;
            if (PhotonNetwork.LocalPlayer.CustomProperties.TryGetValue(MultiplayerConstants.AVATAR_SELECTION_NUMBER,out Avatar_Selection_Number ))
            {
                Debug.Log("Avatar selection number: "+ (int)Avatar_Selection_Number);
                photonView.RPC("InitializeSelectedAvatarModel",RpcTarget.AllBuffered,(int)Avatar_Selection_Number);
            }
    ...
    [PunRPC]
    public void InitializeSelectedAvatarModel(int Avatar_Selection_Number)
    {
        GameObject selectedAvatarGameobject = Instantiate(AvatarModelPrefabs[Avatar_Selection_Number], LocalXRRigGameobject.transform);
